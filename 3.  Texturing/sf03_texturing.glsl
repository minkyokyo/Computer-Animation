...
// Texture
uniform sampler2D tex; // 이게 텍스처임용
...
{
	// Texture
	vec4 color = texture(tex,texcoord); // 2차원 텍스처에서 텍스처좌표를 reference해서 컬러를 가져온당.

	// Lighting
	outColor.rgb = phongReflectionModel(L0, position, normalize(normal))*color.rgb; // phongReflectionModel -> intensity계산.
	outColor.a = color.a;
}
